      ...
      `);
            db.transaction(() => {
                for (const phone of oldData.phones) {
                    insertPhone.run(phone.id, phone.brand, phone.model, String(phone.price), phone.ram || '', phone.storage || '', phone.condition, JSON.stringify(phone));
                }
            })();
...
async function startServer() {
    ...
    const handleBase64Image = (base64Str, id, suffix) => {
        ...
        `);
                for (const phone of newPhones) {
                    ...
                    if (phone.images && Array.isArray(phone.images)) {
                        phone.images = phone.images.map((img, idx) => handleBase64Image(img, phone.id, `img_${idx}`));
                    }
                    if (phone.image) {
                    ...
                    insert.run(phone.id, phone.brand, phone.model, String(phone.price), phone.ram || '', phone.storage || '', phone.condition, sizeKb, finalJson);
                }
            })();
